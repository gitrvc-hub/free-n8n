...
export async function downloadBackup(key: string): Promise<Buffer> {
	const response = await getS3().send(new GetObjectCommand({ Bucket: env.E2_BUCKET, Key: key }));
...
export async function deleteBackup(key: string): Promise<void> {
	await getS3().send(new DeleteObjectCommand({ Bucket: env.E2_BUCKET, Key: key }));
}
