...
export const POST: RequestHandler = async (event) => {
	...
	if (!user || user.status !== 'reclaimed') {
		return json(
			{ error: { code: 'BAD_REQUEST', message: 'Not in reclaimed state' } },
			{ status: 400 }
		);
	}
	...
	try {
		...
		await db.jobLog.create({
			data: {
				...
				userId: user.id,
				status: 'success',
				metadata: { backupId: backup.id, workflowCount: data.workflows.length }
			}
		});
		...
		return json(
			{ error: { code: 'RESTORE_FAILED', message: 'Failed to restore. Try again.' } },
			{ status: 500 }
		);
	}
};
