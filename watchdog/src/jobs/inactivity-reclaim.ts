...
import { getInactiveUsers, markUserReclaimed, createBackupRecord, createJobLog } from '../services/platform-db.js';
...
async function reclaimUser(user: { id: string; email: string; n8nUserId: string }): Promise<void> {
  ...
  try {
    ...
    await createJobLog('reclaim', user.id, 'success', { backupKey: key, backupSize: size, workflowCount: workflows.length });
