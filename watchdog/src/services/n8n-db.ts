...
export async function getAllUserActivity(): Promise<Array<{ n8nUserId: string; lastExecution: Date | null }>> {
...
export async function getUserWorkflows(n8nUserId: string): Promise<Array<{ id: string; name: string; nodes: unknown; connections: unknown; settings: unknown; active: boolean }>> {
  const result = await pool.query(
...
export async function deleteUserData(n8nUserId: string): Promise<void> {
  await pool.query(`DELETE FROM execution_entity WHERE id IN (SELECT ee.id FROM execution_entity ee JOIN workflow_entity we ON ee."workflowId" = we.id JOIN shared_workflow sw ON we.id = sw."workflowId" WHERE sw."userId" = $1)`, [n8nUserId]);
  await pool.query(`DELETE FROM workflow_entity WHERE id IN (SELECT "workflowId" FROM shared_workflow WHERE "userId" = $1)`, [n8nUserId]);
